...
const BRANDS = [
  ...
  '半亩花田','MAIA ACTIVE','bosie','URBAN REVIVO'
].join('、');
  ...
  "items": [
    {
      ...
      "market_signal": "这个动作对竞品/行业意味着什么？从市场营销视角给出1-2句专业判断",
      "crisis_level": null
    }
  ]
}
...
要求：
- trend_forecast 输出3条，覆盖短中长期
- items 输出8-10条情报，尽量覆盖不同品牌和不同category
- 危机舆情类条目需填写 crisis_level（轻微/中度/严重）
- facts 数组每条4个字段，label根据category类型灵活调整
- 所有内容必须基于真实搜索结果，不可编造
- market_signal 要有专业市场洞察，不能是事件复述`;
...
async function main() {
  ...
  try {
    ...
    // 提取JSON（去除可能的markdown包裹）
    const start = raw.indexOf('{');
    const end = raw.lastIndexOf('}');
    if (start === -1) throw new Error('未找到JSON内容，原始输出：\n' + raw.slice(0,500));
